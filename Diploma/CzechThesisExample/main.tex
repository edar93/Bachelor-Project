...
\section{Úvod}
\subsection{Cíle bakalářské práce}
...
\subsubsection{Aplikační servery - Tomcat}
Co vse ?
...
\subsubsection{Karma}
Chcu zminit ale neni co smazat ?

Karma je test runner pro spouštění testů v Node.js. Využívá prohlížeč, v němž spouští testy.
...
Hra je vyvinuta ve Springu. Springovský projekt má tuto základní strukturu: \\
src  - zakladni složka s kódem
...
Diagram s entitami - překryvaji se ?
Moc zoufale ?

\begin{figure}[H]
\centering\includegraphics[width=0.7\textwidth]{./Figures/DB model.png}\caption{DB model oracle}
\end{figure}

\subsection{Spring sec, Bcrypt}
moc dlouhe ?

\lstset{language=XML}
\begin{lstlisting}[caption=Spring security konpigurace]
...
<security:authentication-provider>
	<security:jdbc-user-service
		...
		users-by-username-query="SELECT login AS username, password, enabled FROM players WHERE login=?"
		authorities-by-username-query="SELECT r.login as username, r.user_role as role  FROM players p join USER_ROLES r on p.LOGIN = r.login and p.login=?"/>
